...
function printSnapshotSummary(s: Awaited<ReturnType<typeof computeAtlasSnapshot>>) {
  ...
  console.log(`    ${h.parallelsTotal} parallels (median weight ${h.parallelsMedianWeight ?? 'n/a'}, p90 ${h.parallelsP90Weight ?? 'n/a'}) · ${h.contrasts} contrasts`);
  console.log(`  Top pairs (by median weight):`);
  for (const m of s.traditionMatrix.slice(0, 5)) console.log(`    ${m.a} ↔ ${m.b}: ${m.parallels} parallels, median weight ${m.medianWeight ?? 'n/a'}`);
