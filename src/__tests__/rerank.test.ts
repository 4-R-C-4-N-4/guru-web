...
// Score calibration (todo:19ea34ea, §8.1). Two env-gated levers on the VECTOR
// similarity term, both default-off, both proving: (a) they move ranking in the
// intended direction, (b) they are byte-identical no-ops at strength 0 / absent,
// and (c) they leave the graph/lexical rescue legs (similarity 0) untouched.
function vchunk(id: string, tradition: string, textId: string, distance: number, tokenCount: number): RetrievedChunk {
  return {
    id, text_id: textId, tradition, text_name: 'tn', section: 's',
    translator: null, body: 'b', token_count: tokenCount, source: 'vector', distance,
  };
}

describe('mergeAndRerank — length normalization (todo:19ea34ea §8.1)', () => {
  // long is the RAW-similarity winner (distance 0.28 → sim 0.72 vs 0.70); length
  // norm damps its 100-token mass and lifts the 1-token short chunk past it.
  const long = vchunk('long', 'x', 'A', 0.28, 100);
  const short = vchunk('short', 'y', 'B', 0.30, 1);

  it('absent lengthNorm, the raw-similarity order stands (long first)', () => {
    expect(mergeAndRerank([long, short], [], 5).map(c => c.id)).toEqual(['long', 'short']);
  });

  it('length norm damps the long chunk so the short one wins (fluency inversion fix)', () => {
    const out = mergeAndRerank([long, short], [], 5, { lengthNorm: { b: 0.5, avgTokens: 50 } });
    expect(out[0].id).toBe('short');
  });

  it('b=0 is an exact no-op (factor 1) — default neutrality', () => {
    const out = mergeAndRerank([long, short], [], 5, { lengthNorm: { b: 0, avgTokens: 50 } });
    expect(out.map(c => c.id)).toEqual(['long', 'short']);
  });
});

describe('mergeAndRerank — hub-frequency dampening (todo:19ea34ea §8.1)', () => {
  // central is the raw winner; hub gives it centrality 1.0 (a "matches
  // everything" work) vs 0.0 for the peripheral primary, which then overtakes it.
  const central = vchunk('central', 'x', 'HUB', 0.28, 1); // sim 0.72
  const primary = vchunk('primary', 'y', 'RARE', 0.30, 1); // sim 0.70
  const hub = new Map([['HUB', 1.0], ['RARE', 0.0]]);

  it('absent hubDampen, the central work wins on raw similarity', () => {
    expect(mergeAndRerank([central, primary], [], 5).map(c => c.id)).toEqual(['central', 'primary']);
  });

  it('hub dampening discounts the central work so the peripheral primary rises', () => {
    const out = mergeAndRerank([central, primary], [], 5, { hubDampen: { beta: 0.5, hub } });
    expect(out[0].id).toBe('primary');
  });

  it('beta=0 is an exact no-op — default neutrality', () => {
    const out = mergeAndRerank([central, primary], [], 5, { hubDampen: { beta: 0, hub } });
    expect(out.map(c => c.id)).toEqual(['central', 'primary']);
  });

  it('leaves a graph-only rescue hit (similarity 0) untouched — protects the §8.2/§8.3 legs', () => {
    // A strong central vector hub and a graph-only rescue in a rare tradition.
    // Baseline: the hub outranks the rescue. Under heavy hub dampening the hub's
    // vector term collapses while the graph term is untouched, so the rescue wins
    // — proving the lever cannot suppress the graph leg it is meant to unlock.
    const graphRescue = chunk('rescue', 'rare', { source: 'graph', tier: 'verified' });
    graphRescue.text_id = 'HUB'; // even mapped to the hub, a sim-0 hit is exempt
    const base = mergeAndRerank([central], [graphRescue], 5).map(c => c.id);
    expect(base[0]).toBe('central');
    const out = mergeAndRerank([central], [graphRescue], 5, { hubDampen: { beta: 0.95, hub } }).map(c => c.id);
    expect(out[0]).toBe('rescue');
  });
});
